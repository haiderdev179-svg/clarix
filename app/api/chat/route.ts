...
export async function POST(request: Request) {
  ...
  try {
    ...
    const modelToBeUsed = MODEL_REGISTRY[selectedModel as ModelId];
    ...
    //todo: caching
      ...
      const hasSubscription = data.result.items.length > 0
      
      if(!hasSubscription){
        return new Response("Forbidden: This model needs subscription☹️", { status: 403 })
      };
    }
